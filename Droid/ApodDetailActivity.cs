...
namespace HuePod.Droid
{
	...
	public class ApodDetailActivity : Activity
	{
			...
			if (item.ItemId == Resource.Id.save_apod)
			{
				...
				if (permission != Permission.Granted)
				{
					...
					ActivityCompat.RequestPermissions(
						this, new string[] { Manifest.Permission.WriteExternalStorage }, SaveStoragePermission);
				}
				else
				{
					var connections = Plugin.Connectivity.CrossConnectivity.Current.ConnectionTypes;
					if (!connections.Any(c => c == Plugin.Connectivity.Abstractions.ConnectionType.WiFi))
					{
						var promptConfig = new Acr.UserDialogs.ConfirmConfig {
							CancelText = "Cancel",
							OkText = "Continue",
							Message ="Download anyway?"
						};
						promptConfig.OnConfirm += DownloadImage;
						Acr.UserDialogs.UserDialogs.Instance.Confirm(promptConfig);
					}
					else
					{
						DownloadImage();
		...
		public void DownloadImage(bool download = true)
		{
			if (download)
			{
		...
		{
			if (SaveStoragePermission == requestCode)
			{
				Toast.MakeText(this, "Press download again", ToastLength.Short).Show();
			}
			else {
